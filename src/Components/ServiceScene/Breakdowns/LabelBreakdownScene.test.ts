...
describe('buildLabelsQuery', () => {
  test('should build no-parser query with no filters', () => {
    ...
    expect(result).toMatchObject({
      expr: `sum(count_over_time({\${filters} ,cluster != ""}  \${metadata} \${levels} \${patterns} \${lineFilterV2}  \${fields} [$__auto])) by (${VAR_LABEL_GROUP_BY_EXPR})`,
    });
  });
  ...
  test('should build logfmt-parser query with structured metadata filters', () => {
    ...
    const result = buildLabelsQuery({} as SceneObject, VAR_LABEL_GROUP_BY_EXPR, 'cluster');
    expect(result).toMatchObject({
      expr: `sum(count_over_time({\${filters} ,cluster != ""}  \${metadata} \${levels} \${patterns} \${lineFilterV2} | logfmt  \${fields} [$__auto])) by (${VAR_LABEL_GROUP_BY_EXPR})`,
    });
  });
  test('should build json-parser query with structured metadata filters', () => {
    ...
    const result = buildLabelsQuery({} as SceneObject, VAR_LABEL_GROUP_BY_EXPR, 'cluster');
    expect(result).toMatchObject({
      expr: `sum(count_over_time({\${filters} ,cluster != ""}  \${metadata} \${levels} \${patterns} \${lineFilterV2} | json | drop __error__, __error_details__  \${fields} [$__auto])) by (${VAR_LABEL_GROUP_BY_EXPR})`,
    });
  });
  test('should build mixed-parser query with structured metadata filters', () => {
    ...
    const result = buildLabelsQuery({} as SceneObject, VAR_LABEL_GROUP_BY_EXPR, 'cluster');
    expect(result).toMatchObject({
      expr: `sum(count_over_time({\${filters} ,cluster != ""}  \${metadata} \${levels} \${patterns} \${lineFilterV2} | json | logfmt | drop __error__, __error_details__  \${fields} [$__auto])) by (${VAR_LABEL_GROUP_BY_EXPR})`,
    });
  });
});
