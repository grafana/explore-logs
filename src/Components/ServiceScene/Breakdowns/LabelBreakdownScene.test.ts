...
import { VAR_FIELDS, VAR_LABEL_GROUP_BY_EXPR, VAR_LEVELS } from '../../../services/variables';
...
describe('buildLabelsQuery', () => {
  test('should build no-parser query with no filters', () => {
    ...
    expect(result).toMatchObject({
      expr: `sum(count_over_time({\${filters} ,cluster != ""}  \${${VAR_LEVELS}} \${patterns} \${lineFilter} [$__auto])) by (${VAR_LABEL_GROUP_BY_EXPR})`,
    });
  });
  ...
  test('should build logfmt-parser query with structured metadata filters', () => {
    ...
    const result = buildLabelsQuery({} as SceneObject, VAR_LABEL_GROUP_BY_EXPR, 'cluster');
    expect(result).toMatchObject({
      expr: `sum(count_over_time({\${filters} ,cluster != ""}  \${${VAR_LEVELS}} \${patterns} \${lineFilter} | logfmt  \${fields} [$__auto])) by (${VAR_LABEL_GROUP_BY_EXPR})`,
    });
  });
  test('should build json-parser query with structured metadata filters', () => {
    ...
    const result = buildLabelsQuery({} as SceneObject, VAR_LABEL_GROUP_BY_EXPR, 'cluster');
    expect(result).toMatchObject({
      expr: `sum(count_over_time({\${filters} ,cluster != ""}  \${${VAR_LEVELS}} \${patterns} \${lineFilter} | json | drop __error__, __error_details__  \${fields} [$__auto])) by (${VAR_LABEL_GROUP_BY_EXPR})`,
    });
  });
  test('should build mixed-parser query with structured metadata filters', () => {
    ...
    const result = buildLabelsQuery({} as SceneObject, VAR_LABEL_GROUP_BY_EXPR, 'cluster');
    expect(result).toMatchObject({
      expr: `sum(count_over_time({\${filters} ,cluster != ""}  \${${VAR_LEVELS}} \${patterns} \${lineFilter} | json | logfmt | drop __error__, __error_details__  \${fields} [$__auto])) by (${VAR_LABEL_GROUP_BY_EXPR})`,
    });
  });
});
