...
describe('buildFieldsQueryString', () => {
  test('should build logfmt-parser query', () => {
    ...
    expect(result).toEqual(
      `sum by (caller) (count_over_time({\${filters}}  \${metadata} \${levels} \${patterns} \${lineFilters} | logfmt | caller!="" \${fields} [$__auto]))`
    );
  });
  test('should build json-parser query', () => {
    ...
    const result = buildFieldsQueryString('caller', filterVariable, detectedFieldsFrame);
    expect(result).toEqual(
      `sum by (caller) (count_over_time({\${filters}}  \${metadata} \${levels} \${patterns} \${lineFilters} | json | drop __error__, __error_details__ | caller!="" \${fields} [$__auto]))`
    );
  });
  test('should build mixed-parser query', () => {
    ...
    const result = buildFieldsQueryString('caller', filterVariable, detectedFieldsFrame);
    expect(result).toEqual(
      `sum by (caller) (count_over_time({\${filters}}  \${metadata} \${levels} \${patterns} \${lineFilters} | json | logfmt | drop __error__, __error_details__ | caller!="" \${fields} [$__auto]))`
    );
  });
  test('should build metadata query', () => {
    ...
    expect(result).toEqual(
      `sum by (caller) (count_over_time({\${filters}} | caller!="" \${metadata} \${levels} \${patterns} \${lineFilters}  \${fields} [$__auto]))`
    );
  });
});
